jQuery(document).ready(
  () => {
    let info = JSON.parse(sessionStorage.getItem('info')) || {}, today = new Date(), days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], orderedDate, ondate, seatList = [], restOfSeats = [20, 20, 20, 20, 20], movies = [];
    //根據網域決定路徑
    ...
    jQuery.getJSON(
      ...
      response => {
        ...
        if (info) {
          jQuery("#Movie").val(info.id).change();
        }
      }
    );
    //載入日期選單
    jQuery("#Movie").change(
      function () {
        if (jQuery(this).val()) {
          ...
          ondate = new Date(JSON.parse(sessionStorage.getItem('info'))?.date || movies.find(movie => movie.id === parseInt(jQuery(this).val()))?.date);
          while (date <= ondate) {
            dateList +=
      ...
      event => {
        ...
        for (let i = 1; i <= 20; i++) {
          ...
          if (chosenSeats.includes(`${i}`)) {
            seatTable +=
              `<img src='${API_PATH}/Content/Images/03D03.png' width='40' height='40' />`;
          } else {
            seatTable +=
              `<img src='${API_PATH}/Content/Images/03D02.png' width='40' height='40' />
    ...
    jQuery("#ToStep3").click(
      event => {
        ...
        let seatFilter = seatList.filter(value => value.date === jQuery("#Date").val()),
          id = parseInt(seatFilter.length ? seatFilter.at(-1).number + 1 : `${orderedDate.getFullYear()}${(orderedDate.getMonth() + 1).toString().padStart(2, 0)}${orderedDate.getDate()}0001`);
        jQuery.post(
          `${API_PATH}/Add.ashx`,
          {
            item: 'Ticket',
